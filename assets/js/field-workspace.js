...
async function repairLocalQueue(){const c=await UI.confirm('ตรวจซ่อมคิวส่งข้อมูล?','ระบบจะลบเฉพาะรายการคิวซ้ำและประวัติที่ส่งสำเร็จนานเกิน 7 วัน โดยไม่ลบงานหรือรูปต้นฉบับ','ตรวจซ่อม');if(!c.isConfirmed)return;try{const r=await LocalDraftDB.repairQueue();await openSyncCenter();await refreshSyncStatus();await UI.success('ตรวจซ่อมเรียบร้อย',`ลบคิวซ้ำ ${r.removedDuplicates} รายการ · ล้างประวัติเก่า ${r.removedDone} รายการ`)}catch(e){await UI.error('ตรวจซ่อมไม่สำเร็จ',e.message||'ข้อมูลในเครื่องยังคงอยู่ตามเดิม')}}
